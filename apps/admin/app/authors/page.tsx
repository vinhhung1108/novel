...
function toItems(resp: ListResp): Author[] {
...
export default function AuthorsPage() {
  ...
  useEffect(() => {
    ...
    router.replace(`/authors?${params.toString()}`);
  }, [q, page, limit, router]);
    ...
    (async () => {
      try {
        ...
        if (!res.ok) {
          ...
          setSelected([]); // reset chọn khi đổi list
        }
      } catch (e: any) {
  ...
  async function createOrUpdate() {
    ...
    try {
      const payload = { name: edit.name.trim() };

      let res: Response;
      if (edit.id) {
        // update
        res = await fetch(apiUrl(`/authors/${edit.id}`), {
          method: "PATCH",
          headers: {
        ...
        // create
        res = await fetch(apiUrl("/authors"), {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...getAuthHeader(),
  ...
  async function removeOne(id: string) {
    if (!token) return;
    if (!confirm("Xoá tác giả này?")) return;
  ...
  async function removeBulk() {
    ...
    if (!confirm(`Xoá ${selected.length} tác giả đã chọn?`)) return;
  ...
  const disabledSubmit = !edit.name.trim() || edit.submitting;
  ...
  return (
    <main className="p-6 max-w-6xl mx-auto space-y-5">
      ...
      <div className="text-sm text-gray-700">
        {loading
          ? "Đang tải…"
          : `Tổng: ${total} • Trang ${page}/${Math.max(
              1,
              Math.ceil((total || 0) / (limit || 30))
            )}`}
      </div>
      ...
      <section className={`${CARD} overflow-hidden`}>
        {loading ? (
          ...
          <div className="p-4">Chưa có tác giả.</div>
        ) : (
                  ...
                  <td className="p-3 align-top">
                    <div className="flex gap-3">
                      <button
                        ...
                        onClick={() =>
                          dispatch({ type: "open_edit", payload: a })
                        }
                      >
                        ✏️ Sửa
                      </button>
      ...
      <section className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm">
            Trang {page}/{Math.max(1, Math.ceil((total || 0) / (limit || 30)))}
          </span>
          <button
            onClick={() =>
              setPage((p) =>
            ...
            disabled={
              page >= Math.max(1, Math.ceil((total || 0) / (limit || 30)))
            }
            className="px-3 py-2 rounded-lg border disabled:opacity-50"
          >
            Sau →
          ...
          <button
            onClick={() =>
              setPage(Math.max(1, Math.ceil((total || 0) / (limit || 30))))
            }
            disabled={
              page >= Math.max(1, Math.ceil((total || 0) / (limit || 30)))
            }
      ...
      {edit.open && (
        ...
        >
          <div
            className="w-[min(600px,96vw)] bg-white rounded-xl p-5 grid gap-3"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-semibold m-0">
              {edit.id ? "Sửa tác giả" : "Thêm tác giả"}
            </h3>
